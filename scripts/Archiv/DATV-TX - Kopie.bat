...
if "%FW%"=="no" echo OK, using old F5OEO FW...
if "%FW%"=="no" GoTo DECISIONCODEC
REM if "%FW%"=="yes" if NOT "%DATVOUT%"=="yes" GoTo MOSQUITTO
REM if "%FW%"=="yes" if "%DATVOUT%"=="yes" GoTo DATVRX
...
if "%AUTO%"=="6" %mosquitto% -t %CMD_ROOT%/tx/dvbs2/fec -m %FEC% -h %PLUTOIP%
...
if "%DATVOUT%"=="yes" echo RXL=on > .\ini\rxonoff.ini
...
REM Mosquitto general init commands, don't modify
if "%DATVOUT%"=="yes" set /a SRM = %SR% * 1000
if "%DATVOUT%"=="yes" set /a DIGITALGAIN = 0

if "%DATVOUT%"=="yes" %mosquitto% -t %CMD_ROOT%/tx/dvbs2/tssourceaddress -m %PLUTOIP%:%PLUTOPORT% -h %PLUTOIP%
if "%DATVOUT%"=="yes" %mosquitto% -t %CMD_ROOT%/tx/gain -m %GAIN% -h %PLUTOIP%
if "%DATVOUT%"=="yes" %mosquitto% -t %CMD_ROOT%/tx/mute -m %MUTE% -h %PLUTOIP%
if "%DATVOUT%"=="yes" %mosquitto% -t %CMD_ROOT%/tx/frequency -m %TXFREQUENCY% -h %PLUTOIP%
if "%DATVOUT%"=="yes" %mosquitto% -t %CMD_ROOT%/tx/dvbs2/sr -m %SRM% -h %PLUTOIP%
if "%DATVOUT%"=="yes" %mosquitto% -t %CMD_ROOT%/tx/nco -m %NCO% -h %PLUTOIP%
if "%DATVOUT%"=="yes" %mosquitto% -t %CMD_ROOT%/tx/stream/mode -m %TXMODE% -h %PLUTOIP%
if "%DATVOUT%"=="yes" %mosquitto% -t %CMD_ROOT%/tx/dvbs2/fec -m %FEC% -h %PLUTOIP%
if "%DATVOUT%"=="yes" %mosquitto% -t %CMD_ROOT%/tx/dvbs2/constel -m %MODE% -h %PLUTOIP%
if "%DATVOUT%"=="yes" %mosquitto% -t %CMD_ROOT%/tx/dvbs2/pilots -m %PILOTS% -h %PLUTOIP%
if "%DATVOUT%"=="yes" %mosquitto% -t %CMD_ROOT%/tx/dvbs2/frame -m %FRAME% -h %PLUTOIP%
if "%DATVOUT%"=="yes" %mosquitto% -t %CMD_ROOT%/tx/dvbs2/fecmode -m %FECMODE% -h %PLUTOIP%
if "%DATVOUT%"=="yes" %mosquitto% -t %CMD_ROOT%/tx/dvbs2/agcgain -m %AGCGAIN% -h %PLUTOIP%
if "%DATVOUT%"=="yes" %mosquitto% -t %CMD_ROOT%/tx/dvbs2/digitalgain -m %DIGITALGAIN% -h %PLUTOIP%
if "%DATVOUT%"=="yes" %mosquitto% -t %CMD_ROOT%/tx/dvbs2/fecrange -m %FECRANGE% -h %PLUTOIP%
if "%DATVOUT%"=="yes" %mosquitto% -t %CMD_ROOT%/tx/dvbs2/tssourcemode -m %TSSOURCEMODE% -h %PLUTOIP%
if "%DATVOUT%"=="yes" %mosquitto% -t %CMD_ROOT%/tx/dvbs2/tssourcefile -m %TSSOURCEFILE% -h %PLUTOIP%
if "%DATVOUT%"=="yes" %mosquitto% -t cmd/longmynd/lnb_supply -m %LNBSUPPLY% -h %PLUTOIP%
if "%DATVOUT%"=="yes" %mosquitto% -t cmd/longmynd/polarisation -m %LNBPOL% -h %PLUTOIP%
if "%DATVOUT%"=="yes" %mosquitto% -t cmd/longmynd/swport -m %TUNERPORT% -h %PLUTOIP%

REM Start Control, decoder and MQTT Browser
if "%DATVOUT%"=="yes" start "CONTROL" .\scripts\CONTROL.bat
if "%DATVOUT%"=="yes" start .\Mosquitto\MQTT-Explorer-0.4.0-beta1.exe
if "%DATVOUT%"=="yes" if NOT "%RELAY%"=="on" if "%RXPRG%"=="ffplay" start "FFPLAY" .\scripts\START-FFPLAY-LONGMYND.bat
if "%DATVOUT%"=="yes" if NOT "%RELAY%"=="on" if "%RXPRG%"=="mpv" start "MPV" .\scripts\START-MPV-LONGMYND.bat

if "%DATVOUT%"=="yes" if NOT "%RELAY%"=="on" GoTo DECISIONCODEC
...
if "%FW%"=="yes" %mosquitto% -t %CMD_ROOT%/tx/dvbs2/fec -m %FEC% -h %PLUTOIP%
